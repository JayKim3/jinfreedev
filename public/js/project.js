...
$('.project__portfolio-marvel').mouseenter(function () {
    $('.portfolio-list-title-marvel').text('바닐라 자바스크립트로 100% 구현하였으며 간단한 HTML과 CSS로 디자인적인 부분도 입혀 자바스크립트와 친해지는 계기가 된 프로젝트');
...
$('.project__portfolio-escaperoom').mouseenter(function () {
    $('.portfolio-list-title-escaperoom').text('바닐라 자바스크립트 위주로 주로 이미지를 이용하여 이벤트 핸들러 함수와 콜백함수 내에 작성하여 간단한 애니메이션 효과를 넣어 만든 게임');
...
$('.project__portfolio-tetris').mouseenter(function () {
    $('.portfolio-list-title-tetris').text('자바스크립트 공부가 더 필요하다 생각하여 이번에는 es5문법을 사용하여 구현 / 최대한 혼자 테트리스 알고리즘을 생각하고 구현을 하려 시도 - 방향전환 부분에서 막혀 오픈소스들을 참고하여 코드를 참고하여 완성');
...
$('.project__portfolio-hollyscoffee').mouseenter(function () {
    $('.portfolio-list-title-hollyscoffee').text('CSS를 다른 기술을 사용해보기 위하여 SCSS 강의를 듣고 간단하게 메인 페이지만 만들어봄 - 따로 반응형은 적용하지 않아 모바일 화면에서는 레이아웃이 맞지 않습니다.');
...
$('.project__portfolio-reorder').mouseenter(function () {
    $('.portfolio-list-title-reorder').text('비콘을 이용한 주문예약 시스템을 주제로 서버라는 개념을 처음 알게되어 전체적인 기획 과 Node 서버 구축(사용자와 웹쪽 REST API방식으로 데이터 처리)를 100% 맡았으며 리액트 라이브러리를 사용하여 관리자 웹을 구축 및 사용자의 안드로이드쪽은 다른 두 친구가 개발을 진행 ');
...
$('.project__portfolio-reactstudy').mouseenter(function () {
    $('.portfolio-list-title-reactstudy').text('리액트를 다루는 기술 책으로 매주 토요일 스터디를 2달간 진행하였으며 To-do-list 미니프로젝트를 서로 커스터마이징 하여 제작하고 코드 리뷰를 통하여 리액트의 효율적인 컴포넌트 관리에 대하여 다시 생각해보게된 계기');
...
$('.project__portfolio-swot').mouseenter(function () {
    $('.portfolio-list-title-swot').text('학교 강의실 예약과 동시에 스터디 모집을 연동을 주제로 총 4명에서 팀을 이루었으며 웹, 서버, 앱으로 개발하였으며 전체적인 기획과 웹 부분을 전담하여 담당하였습니다.');
...
$('.project__portfolio-sjinmv').mouseenter(function () {
    $('.portfolio-list-title-sjinmv').text('서비스 준비중 입니다.');
